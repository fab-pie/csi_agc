...
#include "zoecare/tx/pages.h"
#include "zoecare/tx/router_globals.h"
#include "zc_debug.h"
...
static esp_err_t index_get_handler(httpd_req_t *req)
{
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ZC_LOG_SENS(TAG, "Found URL query => %s", buf);
            ...
            if (httpd_query_key_value(buf, "ap_ssid", param1, sizeof(param1)) == ESP_OK) {
                ZC_LOG_SENS(TAG, "Found URL query parameter => ap_ssid=%s", param1);
                preprocess_string(param1);
                if (httpd_query_key_value(buf, "ap_password", param2, sizeof(param2)) == ESP_OK) {
                    ZC_LOG_SENS(TAG, "Found URL query parameter => ap_password=%s", param2);
            ...
            if (httpd_query_key_value(buf, "ssid", param1, sizeof(param1)) == ESP_OK) {
                ZC_LOG_SENS(TAG, "Found URL query parameter => ssid=%s", param1);
                preprocess_string(param1);
                if (httpd_query_key_value(buf, "password", param2, sizeof(param2)) == ESP_OK) {
                    ZC_LOG_SENS(TAG, "Found URL query parameter => password=%s", param2);
                    preprocess_string(param2);
                    if (httpd_query_key_value(buf, "ent_username", param3, sizeof(param3)) == ESP_OK) {
                        ZC_LOG_SENS(TAG, "Found URL query parameter => ent_username=%s", param3);
                        preprocess_string(param3);
                        if (httpd_query_key_value(buf, "ent_identity", param4, sizeof(param4)) == ESP_OK) {
                            ZC_LOG_SENS(TAG, "Found URL query parameter => ent_identity=%s", param4);
            ...
            if (httpd_query_key_value(buf, "staticip", param1, sizeof(param1)) == ESP_OK) {
                LOGI("Found URL query parameter => staticip=%s", param1);
                preprocess_string(param1);
                if (httpd_query_key_value(buf, "subnetmask", param2, sizeof(param2)) == ESP_OK) {
                    LOGI("Found URL query parameter => subnetmask=%s", param2);
                    preprocess_string(param2);
                    if (httpd_query_key_value(buf, "gateway", param3, sizeof(param3)) == ESP_OK) {
                        LOGI("Found URL query parameter => gateway=%s", param3);
                        preprocess_string(param3);
                        int argc = 4;
                        char* argv[4];
...
httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        // Set URI handlers
        LOGI("Registering URI handlers");
        httpd_register_uri_handler(server, &indexp);
        return server;
    }

    LOGI("Error starting server!");
    return NULL;
}
