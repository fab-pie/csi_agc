...
#include "zoecare/tx/cmd_router.h"
#include "zc_debug.h"
...
int set_sta(int argc, char **argv)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            err = nvs_commit(nvs);
            if (err == ESP_OK) {
                ZC_LOG_SENS(TAG, "STA settings %s/%s stored.", set_sta_arg.ssid->sval[0], set_sta_arg.password->sval[0]);
            }
        }
    }
        }
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            ...
            if (err == ESP_OK) {
              err = nvs_commit(nvs);
                if (err == ESP_OK) {
                    LOGI("STA Static IP settings %s/%s/%s stored.", set_sta_static_arg.static_ip->sval[0], set_sta_static_arg.subnet_mask->sval[0], set_sta_static_arg.gateway_addr->sval[0]);
                }
            }
        }
    }
...
esp_err_t set_mac(const char *key, const char *interface, int argc, char **argv) {
    ...
    if (err == ESP_OK) {
        err = nvs_commit(nvs);
        if (err == ESP_OK) {
            ZC_LOG_SENS(TAG, "%s mac address %02X:%02X:%02X:%02X:%02X:%02X stored.", interface, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
        }
    }
    nvs_close(nvs);
    return err;
...
int set_ap(int argc, char **argv)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK) {
            err = nvs_commit(nvs);
            if (err == ESP_OK) {
                ZC_LOG_SENS(TAG, "AP settings %s/%s stored.", set_ap_args.ssid->sval[0], set_ap_args.password->sval[0]);
            }
        }
    }
    nvs_close(nvs);
...
int set_ap_ip(int argc, char **argv)
{
    ...
    if (err == ESP_OK) {
        LOGI("AP IP address %s stored.", set_ap_ip_arg.ap_ip_str->sval[0]);
    }
    nvs_close(nvs);
    return err;
}
...
static int show(int argc, char **argv)
{
    ...
    ZC_IF_DEBUG_MON {
        ...
        printf("STA SSID: %s Password: **** Enterprise: %s %s\n",
            ssid != NULL ? ssid : "<undef>",
            ((ent_username != NULL) && (strlen(ent_username) > 0)) ? ent_username : "<not active>",
            ((ent_username != NULL) && (strlen(ent_username) > 0) && (ent_identity != NULL) && (strlen(ent_identity) > 0)) ? ent_identity : ""
        );
        printf("AP SSID: %s Password: ****\n", ap_ssid != NULL ? ap_ssid : "<undef>");
    }
    ip4_addr_t addr;
    addr.addr = my_ap_ip;
